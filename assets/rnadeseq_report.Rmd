...
# Function to negate the %in% operator
`%nin%` <- Negate(`%in%`)
# Function to get biotypes from gtf file and add them to the DESeq2 results table
get_biotypes <- function(gtf, df) {
    if ("gene_id" %in% colnames(gtf) && "gene_biotype" %in% colnames(gtf)) {
        gtf_map <- subset(gtf, select = c("gene_id", "gene_biotype"))
        colnames(gtf_map)[which(names(gtf_map) == "gene_id")] <- "Ensembl_ID"
        gtf_map_unique <- gtf_map[-which(duplicated(gtf_map)), ]
        return(join(df, gtf_map_unique, by = "Ensembl_ID", type = "left"))
    }
    if ("gene_biotype" %nin% colnames(gtf)) {
        print("No gene biotype available in gtf")
        return(df)
    }
}
...
if (software_versions_csv) {
    if (params$input_type == "featurecounts") {
        ...
        primary_pipeline <- "nf-core/rnaseq"
    }
} else {
    if (params$input_type == "rsem") {
        ...
        primary_pipeline <- "nf-core/smrnaseq"
    }
}
```
...
df <- as.data.frame(lapply(df, function(x) if (is.character(x)) trimws(x) else x))

# Import the .tsv file downloadable from https://portal.qbic.uni-tuebingen.de/portal/web/qbic/user-database -> Projects
projectcode = ifelse(typeof(df$Sub.Project)=='logical', '', as.character(df$Sub.Project))
...
PI = ifelse(typeof(df$Principal.Investigator)=='logical', '', as.character(df$Principal.Investigator))
PI_address = ifelse(typeof(df$PI.Address)=='logical', '', as.character(df$PI.Address))
PI_email = ifelse(typeof(df$PI.E.Mail)=='logical', '', as.character(df$PI.E.Mail))
PI_institute = ifelse(typeof(df$PI.Institute)=='logical', '', as.character(df$PI.Institute))
PI_organization = ifelse(typeof(df$PI.Organization)=='logical', '', as.character(df$PI.Organization))
PM = ifelse(typeof(df$Project.Manager)=='logical', '', as.character(df$Project.Manager))
PM_email = ifelse(typeof(df$Manager.E.Mail)=='logical', '', as.character(df$Manager.E.Mail))
description = ifelse(typeof(df$Description)=='logical', '', as.character(df$Description))
```
...
Otfried-Müller-Straße 37, 72076 Tuebingen
...
```{r processing, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
if(file.exists(QAfile)){
    ...
    df_QA$sampleName = paste(df_QA$`QBiC Code`, df_QA$`Secondary Name`, sep="_")

    #add "Sample Type" only if present in metadata.tsv
    if ("Sample Type" %in% colnames(df_QA)) {
            df_QA[,c("Sample Type")] <- df_QA[,c("Sample Type")]
    }
...
\
")
if ("RIN" %in% colnames(df_QA)){df_QA$RIN <- NULL}
DT::datatable(df_QA, options = list(scrollX  = "100%", scrollY = "400px", pageLength = -1, dom = 'ft'))
```
...
## Load Rsem count files
    if (params$input_type == "rsem") {
        ...
        # to account for more complex,e.g. nested and or full interaction term design
        if(grepl("*|:",design[1])) {
        ...
        row.names(cds@colData) <- paste0(row.names(cds@colData), '_', metadata$Secondary.Name)
        # get raw counts
...
In the first step, reads were aligned and assigned to genomic features (e.g. genes) using nf-core pipelines.
In the next step, the gene counts were analyzed for differential expression with the secondary analysis pipeline rnadeseq.
The workflow highlights the pipelines and methods that were used in colors.

```{r mermaid, echo=F, message=F, warning=F}
library(DiagrammeR)

# Colors
color_nfcore_rnaseq <- if (params$input_type != 'smrnaseq') "#24B064" else "#f0f0f0"
color_nfcore <- if (params$input_type == 'smrnaseq') "#24B064" else "#f0f0f0"

color_featurecounts <- if (params$input_type == 'featurecounts') "#f8c383" else "#f0f0f0"
color_salmon <- if (params$input_type == 'salmon') "#f8c383" else "#f0f0f0"
color_rsem <- if (params$input_type == 'rsem') "#f8c383" else "#f0f0f0"
color_mirtop <- if (params$input_type == 'smrnaseq') "#f8c383" else "#f0f0f0"

# Generate DOT string with conditional color
dot_code <- sprintf('
digraph flowchart {
    graph [layout = dot, rankdir = LR]

    node [shape = box, style = filled, fontname = Helvetica, fontcolor = black]

    A [label = "Primary Bioinformatics\\nnf-core/rnaseq", fillcolor = "%s"]
    B [label = "Primary Bioinformatics\\nnf-core/smrnaseq", fillcolor = "%s"]
    C [label = "Quantification", shape = diamond, fillcolor = "#f8c383", fontcolor = "#333333" width = 1.5, height = 1.5 ]
    D [label = "featureCounts", fillcolor =  "%s", fontcolor = "#333333"]
    E [label = "STAR-Salmon", fillcolor =  "%s", fontcolor = "#333333"]
    F [label = "RSEM", fillcolor =  "%s", fontcolor = "#333333"]
    G [label = "SAMtools", fillcolor =  "%s", fontcolor = "#333333"]
    H [label = "Secondary Bioinformatics\\nqbic-pipelines/rnadeseq", fillcolor = "#2b64b9", fontcolor=white]
...
', color_nfcore_rnaseq, color_nfcore, color_featurecounts, color_salmon, color_rsem, color_mirtop)
...
```{r primary_bioinformatics, echo=FALSE, message=FALSE, warning=FALSE, results = 'asis'}

# Note: Display subsection (even if no multiqc archive is provided)

if (params$input_type == "featurecounts" || params$input_type == "rsem" || params$input_type == "salmon") {
cat(paste0("***

\

# Primary Bioinformatics {#primary-bioinformatics-rnaseq}


The primary bioinformatic analysis was performed using the [nf-core/rnaseq](https://nf-co.re/rnaseq) pipeline, version ",
as.character(ifelse(software_versions_csv, version["nf-core/rnaseq"], version["Workflow"][[1]]["nf-core/rnaseq"])), ".
Further details can be found under the section [RNASeq methods](#rnaseq-methods)."
))
} else if (params$input_type == "smrnaseq"){
cat(paste0("***
...
# Primary Bioinformatics {#primary-bioinformatics-smrnaseq}


The primary bioinformatic analysis was performed using the pipeline [nf-core/smrnaseq](https://nf-co.re/smrnaseq), version ",
as.character(version["Workflow"][[1]]["nf-core/smrnaseq"]), ".
Further details can be found under the section [smrnaseq methods](#smrnaseq-methods)."
...
In order to analyze the gene counts per sample, the sequencing reads were mapped to a reference genome, and assigned to genes or other genomic regions.
The [MultiQC report](./QC/multiqc_report.html) summarizes the read mapping and gene assignment statistics, amongst other quality control metrics.
The read sequencing quality was analyzed using [FASTQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/). Details about [software versions](./QC/multiqc_report.html#software_versions) of the primary bioinformatic analysis
can be found at the end of the [MultiQC report](./QC/multiqc_report.html).
...
The table below was extracted from the `MultiQC` report and summarizes statistics from the primary bioinformatic analysis.
It displays the number of mapped reads, assigned genes and further quality control criteria.
\
",
...
**Note**: The percentage of duplicate (%) reads with respect to read counts can indicate issues.
High numbers of duplicates at low read counts can indicate low library complexity with technical duplication.
\
",
"\
<br>
...
colnames(metadata) <- c("Sample", "SecondaryName")

# Merge metadata with the table
...
if (mqc_version == 'old_mqc') {
    ...
    colnames(table_complete) <- c("Sample",  "SecondaryName", "ReadNumber", "DuplicateReadsIntercept", "GCcontent", "TrimmedReads", "MappedReads", "AssignedReads")

    table_complete <- table_complete %>%
        transmute(
            Sample = paste(Sample, SecondaryName, sep='_'),
            ReadNumber = cell_spec(round((ReadNumber/1000000), 2),
                                    background="lightblue",
                                    align = "center",
                                    extra_css = "display:block; width:100%;"),
            DuplicateReadsIntercept = ifelse(DuplicateReadsIntercept > 1,
                                            cell_spec(DuplicateReadsIntercept,
                                            color="orange",
                                            bold=T),
                                            cell_spec(DuplicateReadsIntercept,
                                            color="black")),
            GCcontent = round(GCcontent,2),
            TrimmedReads = round(TrimmedReads,2),
            MappedReads = ifelse(MappedReads > 80,
                                cell_spec(MappedReads,
                                            background="lightblue",
                                            align = "center",
                                            extra_css = "display:block; width:100%;"),
                                cell_spec(MappedReads,
    ...
    table_complete <- table_complete %>%
        transmute(
            Sample = paste(Sample, SecondaryName, sep='_'),
            ReadNumber = cell_spec(round((raw_total_sequences/1000000),2),
                                    background="lightblue",
                                    align = "center",
                                    extra_css = "display:block; width:100%;"),
            DuplicateReadsIntercept = ifelse(reads_duplicated_percent > 1,
                                            cell_spec(reads_duplicated_percent,
                                                        color="orange",
                                                        bold=T,
                                                        align = "center",
                                                        extra_css = "display:block; width:100%;"),
                                            cell_spec(reads_duplicated_percent,
                                                        ...
                                                        align = "center",
                                                        extra_css = "display:block; width:100%;")),
            GC_untrimmed = round(GC_untrimmed,2),
            GC_trimmed = round(GC_trimmed,2),
            TrimmedReads = round(percent_trimmed,2),
            MappedReads =  ifelse(uniquely_mapped_percent > 80,
                                cell_spec(uniquely_mapped_percent,
                                            background="lightblue",
                                            align = "center",
                                            extra_css = "display:block; width:100%;"),
                                cell_spec(uniquely_mapped_percent,
            ...
            AssignedReads = ifelse( round(total_assigned_tags/total_tags*100,2) > 60,
                                cell_spec( round(total_assigned_tags/total_tags*100,1),
                                            background="lightblue",
                                            align = "center",
                                            extra_css = "display:block; width:100%;"),
                                cell_spec( round(total_assigned_tags/total_tags*100,2),
                                            background="orange",
                                            align = "center",
                                            extra_css = "display:block; width:100%;"))
        )
...
## Mapping statistics {.tabset .tabset-pills}

The read mapping statistics to the reference genome for each sample are shown below. Typical values for a good mapping are above 80% uniquely mapped reads.
...
\
", ifelse(mqc_version == 'old_mqc', "
## Read assignment statistics {.tabset .tabset-pills}

The plots below display how many reads could be assigned to genes in percentage and actual counts.
In general, most reads should be assigned. Assignment types:
...
The statistics of read assignment to genes are shown below for pipeline runs on data produced by `rnaseq <= v1.4.2`.
If a later version was used, these plots are not generated by the nf-core/rnaseq anymore.
However, you can find `star/featurecounts/<sample>.featureCounts.txt.summary` or `star_salmon/featurecounts/<sample>.featureCounts.txt.summary` files
in the nf-core/rnaseq pipeline output folder which contain the necessary information for creating such plots.
")))
```
...
## Read feature distribution {.tabset .tabset-pills}

The plots below show the distribution of mapped reads and their assigment to genomic features (CDS exon, 5'UTR exon, 3' UTR exon, Intron, Intergenic regions).
...
## Read strandedness {.tabset .tabset-pills}

The following plot shows the number/percentage of reads that match the strandedness of overlapping transcripts. It can be used to infer whether RNA-seq library preps are stranded (sense or antisense).
...
The primary bioinformatic pipeline `r primary_pipeline` was used to quantify the reads and assign them to genomic features.
See further details in the [computational methods](#computational-methods).

### Raw and normalized count tables

The raw count table from the [primary bioinformatic pipeline](#primary-bioinformatics-rnaseq) `r primary_pipeline` and normalized count tables are available [here](./differential_gene_expression/gene_counts_tables).
The [secondary bioinformatic analysis](#secondary-pipeline) of differential expression used the raw gene count table as input.
For a PCA analysis and heatmap visualizations, the `r norm_method_long_text` normalized gene counts were used.
`r norm_method_override_text`
`r nsub_genes_text`

### Principal component analysis (PCA)

A PCA analysis was conducted to visualize the clustering of samples according to their gene expression.
This visual representation helps to understand overall effects of experimental conditions, sample grouping and detect possible technical batch effects.
The PCA plot was generated based on the `r norm_method_text` normalized gene expression.
...
cat("As batch effects could be observed in the PCA, they were corrected for.
...
A sample distance heatmap was generated to reveal similarities and differences in gene expression across all samples.
Two samples have more similar gene expression if their distance is closer to 0 (darker blue).
The heatmap is based on Euclidean distances of the normalized expression values for all samples.
...
# Secondary Bioinformatics {#secondary-pipeline}

The seconday analysis of differential gene expression was performed using the [QBiC pipeline rnadeseq](https://github.com/qbic-pipelines/rnadeseq),
version `r as.character(params$revision)`.

## Differential expression (DE) results

The `report.zip` archive contains the results of the [differential_gene_expression](./differential_gene_expression).
After extracting the ZIP archive the `report` contains the following subfolders:
...
Click to expand/uncollapse the details:

<details>
<summary>**differential_gene_expression**<br></summary>
- **rnadeseq_report.html**
- **metadata:**
    - `DESeq2_coefficients.tsv`: table with the coefficients of the linear model used for DE analysis.
    ...
    - `requested_gene_list.txt`: provided list of interesting genes (if provided).
- **gene_counts_tables:**
...
- **plots:**
    ...
    - `PCA_*.{pdf,png,svg}`: PCA plots of the `r norm_method_text` normalized gene counts.
    ...
    - `volcano_plots`: volcano plots for each of the contrasts in the analysis.
</details><br>

In differential gene expression analysis, the gene expression data of two or more sample groups is compared.
These comparisons are called _contrasts_. For the current report, the following contrasts or comparisons were made:
...
if (isProvided(params$path_contrast_matrix)){
    ...
    for (i in c(1:ncol(contrasts))) {
        ...
        if (isProvided(params$path_gtf)) { DE_genes_contrast_genename <- get_biotypes(gtf, DE_genes_contrast_genename) }
...
if (isProvided(params$path_contrast_list)) {
    ...
    for (i in c(1:nrow(contrasts))) {
        ...
        current_contrast_name  <- paste0(strsplit(sapply(strsplit(cont[1], '_'), function(x) paste(x[2])), '_')[2], ": ", cont[2], "_vs_", cont[3])
        results_DEseq_contrast <- results(cds, contrast=cont)
        results_DEseq_contrast <- as.data.frame(results_DEseq_contrast)
        print(current_contrast_name)
...
if (!isProvided(params$path_contrast_matrix) & !isProvided(params$path_contrast_list) & !isProvided(params$path_contrast_pairs)) {
    ...
    for (contname in contrast_names) {
        ...
        results_DEseq_contrast <- as.data.frame(results_DEseq_contrast)
        contrast_len <- sapply(strsplit(contname, "_"), length)
        current_contrast_name  <- paste0(sapply(strsplit(contname, '_'), function(x) paste(x[3:contrast_len], collapse ="")))
        print(current_contrast_name)
        print("Analyzing contrast:")
        print(contname)
...
if (isProvided(params$path_gtf)) { DE_genes_final_table <- get_biotypes(gtf, DE_genes_final_table) }
...
```{r print_contrasts, eval=F}
for (i in 1:length(contrast_names)){
    contrast_len <- sapply(strsplit(contrast_names[i], '_'), length)
    current_condition <- sapply(strsplit(contrast_names[i], '_'), function(x) paste(x[2], collapse=' '))
    current_contrast_name <- sapply(strsplit(contrast_names[i], '_'), function(x) paste(x[(contrast_len-2):contrast_len], collapse=' '))
    cat(paste("* Comparison: ", current_condition, ":", current_contrast_name, "\n"))
}
```


```{r, print_contrasts, eval=T, results='asis', warning=FALSE, message=FALSE}
```


The expression profile of the first group (treatment) was compared to the second group (control).
...
The [secondary bioinformatics pipeline](#secondary-pipeline) uses `DESeq2` to analyze the differential gene expression.
...
controls the false discovery rate in multiple testing.

The following linear model (design) was used in DESeq2 to model gene expression:
```{r print_model, results='asis'}

lines <- (readLines(params$path_design))
cat(paste0("`", gsub('condition_', '', lines), "`"), sep = "\n")

```


Expand/uncollapse the table below to show the meaning of different design strategies with example conditions:

<details>
<summary>**Design strategies**</summary>

| Design                           | Interpretation                                                                   |
| -------------------------------- | -------------------------------------------------------------------------------- |
| `~ treatment`                    | Model effect of treatment only                                                   |
| `~ genotype + treatment`         | Model additive effects of treatment and genotype (assumes independence of both ) |
| `~ batch + treatment`            | Control for batch effects when testing treatment                                 |
| `~ genotype * treatment`         | Include interaction — test if treatment effects differ between genotypes         |
| `~ batch + genotype * treatment` | Adjusting for batch and testing interaction effects                              |

</details><br>


In the resulting DE tables, the genes are marked as **DE** if they were found to be
differentially expressed (DE) for any of the compared conditions (contrasts).
...
Genes were considered as differentially expressed when they exhibit a padj <= `r pval_text` (horizontal drawn through line)
and a Log2FC >= `r logFC_text` or <= -`r logFC_text` (vertical grey lines).
Here, volcano plots for all compared conditions (contrasts) are shown. If a list of requested genes was provided as input,
...
The original plots, as well as interactive plots can be found [here](./differential_gene_expression/plots/volcano_plots/).
...
for (file in allgenes_files){
    ...
    current_contrast_name <- sapply(strsplit(fname, '_'), function(x) paste(x[5:contrast_len], collapse=' '))
    contrast_list <- c(contrast_list, current_contrast_name)

    # Prettify contrast in plot title
    # Step 1: Split the filename by underscores
    parts <- strsplit(fname, "_")[[1]]
    n <- length(parts)

    # Step 2: Take the last 4 components and join them with spaces
    current_contrast_name <- paste(parts[(n - 2):n], collapse = " ")

    # Step 3: Check length and optionally insert line break
    if (nchar(current_contrast_name) > 30) {
        # Try splitting after first space
        split_pos <- regexpr(" ", current_contrast_name)[1]
        if (split_pos > 0 && split_pos < nchar(current_contrast_name)) {
        current_contrast_name <- paste0(
            substr(current_contrast_name, 1, split_pos - 1),
            "\n",
            substr(current_contrast_name, split_pos + 1, nchar(current_contrast_name))
        )
        }
    }
    ...
    label_DE <- paste0("DE:", "p_adj<", params$adj_pval_threshold, " & Log2FC >|", params$logFC_threshold, "|")
    label_NS <- "NS"
    label_logFC <-  paste0("log2FC>|", params$logFC_threshold, "|")
    label_padj <- paste0("p_adj< ", params$adj_pval_threshold)

    DE_genes <- DE_genes %>%
            mutate(color_code = ifelse((abs(DE_genes$log2FoldChange) >= params$logFC_threshold & DE_genes$logpval >= -log10(params$adj_pval_threshold)), label_DE,
            ifelse((abs(DE_genes$log2FoldChange) >= params$logFC_threshold & DE_genes$logpval < -log10(params$adj_pval_threshold)), label_logFC,
                ifelse((abs(DE_genes$log2FoldChange) < params$logFC_threshold & DE_genes$logpval >= -log10(params$adj_pval_threshold)), label_padj,
                    ifelse((abs(DE_genes$log2FoldChange) < params$logFC_threshold & DE_genes$logpval < -log10(params$adj_pval_threshold)), label_NS, "purple")
                        )
                    )
                )
            )

    DE_genes$color_code <- as.factor(DE_genes$color_code)
    color_map <- c(
        setNames("#d03a33", label_DE),
        setNames("#458a34", label_logFC),
        setNames("#444444", label_NS),
        setNames("#465adb", label_padj)
    )
    ...
    pg <- ggplot(DE_genes,
            aes(x=log2FoldChange,
                ...
                text=paste("Gene: ", gene_name,
                            "<br>", "Log2FC: ", formatC(log2FoldChange, digits=2),
                            "<br>padj: ", formatC(padj, digits = 2),
                            "<br>", "-Log10padj: ", formatC(truelogpval, digits=2, format='f'),
                            ifelse("gene_biotype" %in% colnames(DE_genes), paste("<br>Biotype:", gene_biotype), ""))
                            )) +
            geom_point(alpha = 0.5,
                        na.rm = TRUE) +
            geom_hline(yintercept = -log10(params$adj_pval_threshold), linewidth = 0.2, color = "grey") +
            geom_vline(xintercept = -params$logFC_threshold, linewidth = 0.2, color = "grey") +
            geom_vline(xintercept = params$logFC_threshold, linewidth = 0.2, color = "grey") +
            ...
            theme(panel.spacing = unit(ifelse(length(allgenes_files)>2, 0.1, 0.6), "lines")) +
            scale_x_continuous(breaks = seq(-200, 200, ifelse(log2FoldChange_quotient<3, 1, 5))) +
            ggtitle(paste('Comparison:', current_contrast_name),
                    subtitle=paste0("DE results; total=",
                        thousandFormat(num_all), ", DE=",
                        thousandFormat(numDE)))


    # Save static plot
    ggsave(filename = file.path(wd, "/differential_gene_expression", "plots", "volcano_plots", paste0(fname, ".png")),
            plot = pg,
            width = 8,
            height = 5)

    pg <- pg + ggtitle(paste('<b>Comparison:', current_contrast_name, '</b>',
                        '<br><sup>',
                        paste0("DE results; total=", thousandFormat(num_all), ", DE=", thousandFormat(numDE)),
                        '</sup>'))

    # Define smaller plotly size
    ppg <- partial_bundle(ggplotly(pg, tooltip = "text")) %>% layout(autosize = F, width = 700, height = 500, margin = list(t = 100))
    ...
    if (isProvided(params$path_genelist)) {
        ...
        p <- EnhancedVolcano(DE_genes,
                    ...
                    pCutoff = params$adj_pval_threshold,
                    FCcutoff = params$logFC_threshold,
                    ...
                    title = paste('Comparison:', current_contrast_name),
                    subtitle = paste0("DE results; total=", thousandFormat(num_all), ", DE=", thousandFormat(numDE)),
                    caption = ''
                    )
...
```{r show_enhanced_volcano, results = 'asis', fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

# Show the volcano plots in tabs
for (i in 1:length(volcano_list)){
    cat("#### Contrast: ", contrast_list[[i]], "\n")
    cat("\n")
    baseline_contrast <- sapply(strsplit(contrast_list[i], ' '), function(x) paste(x[3], collapse=' '))
    experiment_contrast <- sapply(strsplit(contrast_list[i], ' '), function(x) paste(x[1], collapse=' '))
    cat(paste('A positive log2FC indicates an upregulation of a gene in', experiment_contrast, 'compared to', baseline_contrast, '.'))
    cat("\n\n")
...
Plots of the normalized gene counts were generated for some example genes. Count normalization was performed by
...
cat("Plots for the [genes of interest](./differential_gene_expression/metadata/requested_gene_list.txt) can be found [here](./differential_gene_expression/plots/boxplots_requested_genes/).
\
")
```
...
if (isProvided(params$set_background)) {
    ...
    dir.create("enrichment_analysis/metadata")
    write.table(custom_background, "enrichment_analysis/metadata/custom_background.txt", quote=F, row.names = F, col.names = F)
}
...
if (mv_status) {
    dir.create("enrichment_analysis/metadata")
    system("mv *.gmt enrichment_analysis/metadata")
    if (isProvided(params$custom_gmt)) {
        system(paste("cp", params$custom_gmt, "enrichment_analysis/metadata/"))
    }
}
...
# For each contrast do enrichment analysis
...
for (file in contrast_files){
    ...
    dir.create(paste("enrichment_analysis", fname, sep="/"))
    dir.create(paste("enrichment_analysis", fname, "KEGG_pathways", sep="/"))
    dir.create(paste("enrichment_analysis", fname, "enrichment_plots", sep="/"))
    ...
    if (nrow(DE_genes) < 2){
        print(paste0("Not enough DE genes to allow for a enrichment analysis for contrast: ", fname))
        next
    }
    ...
    if (nrow(pathway_gostres) > 0){
        ...
        ggsave(pg, filename = paste0("enrichment_analysis/", "/", fname, "/gost_pathway_gostplot.pdf"),
            device="pdf",
            height=10, width=15, units="cm", limitsize=F)
        ggsave(pg, filename = paste0("enrichment_analysis/", "/", fname, "/gost_pathway_gostplot.png"),
            device="png",
            height=10, width=15, units="cm", dpi=300, limitsize=F)
        ggsave(pg, filename = paste0("enrichment_analysis/", "/", fname, "/gost_pathway_gostplot.svg"),
            device="svg",
            height=10, width=15, units="cm", dpi=300, limitsize=F)
        ...
        if (isProvided(params$set_background)) {
            ...
            ggsave(paste0("enrichment_analysis", "/", fname, "/gost_pathway_venn_diagram.pdf"),pv, device="pdf", width=10, height=15)
            ggsave(paste0("enrichment_analysis", "/", fname, "/gost_pathway_venn_diagram.png"),pv, device="png", width=10, height=15)
            ggsave(paste0("enrichment_analysis", "/", fname, "/gost_pathway_venn_diagram.svg"),pv, device="svg", width=10, height=15)
        }
    }
    ...
    write.table(pathway_gostres_table,
                file = paste0("enrichment_analysis", "/", fname, "/pathway_enrichment_results.tsv"),
    ...
    if (nrow(pathway_gostres) > 0){ #if there are enriched pathways
        ...
        for (df in res){
            ...
            write.table(df_subset,
                        file = paste0("enrichment_analysis", "/", fname, "/", make.names(db_source), "_pathway_enrichment_results.tsv"),
                        sep="\t", quote = F, col.names = T, row.names = F)
            ...
            tryCatch(
                {
                    ggsave(p, filename = paste0("enrichment_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.pdf"), device = "pdf", height = 5+0.5*nrow(df_subset), units = "cm", limitsize=F)
                },
                error=function(e) {
                    print(paste0("Could not save enrichment_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.pdf because of the following error:\n", e))
                }
            )
            tryCatch(
                {
                    ggsave(p, filename = paste0("enrichment_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.png"), device = "png", height = 5+0.5*nrow(df_subset), units = "cm", limitsize=F)
                },
                error=function(e) {
                    print(paste0("Could not save enrichment_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.png because of the following error:\n", e))
                }
            )
            tryCatch(
                {
                    ggsave(p, filename = paste0("enrichment_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.svg"), device = "svg", height = 5+0.5*nrow(df_subset), units = "cm", limitsize=F)
                },
                error=function(e) {
                    print(paste0("Could not save enrichment_analysis", "/", fname, "/enrichment_plots/", make.names(db_source), "_pathway_enrichment_plot.svg because of the following error:\n", e))
                }
            )
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        dir.create(paste("enrichment_analysis", fname, "heatmaps", sep="/"))

                        # Save matrix as table so that the values are accessible if heatmap plotting does not work properly
                        write.table(mat, file=paste0("enrichment_analysis", "/",fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".tsv"), sep="\t", quote=F, row.names=F)

                        # Reformat matrix so that it can be used by pheatmap (throws error if String columns/IDs are in the table)
                        rownames(mat) <- paste0(mat$Ensembl_ID, "_", mat$gene_name)
                        mat$gene_name <- NULL
                        mat$Ensembl_ID <- NULL
                        mat <- data.matrix(mat)
                        ...
                        tryCatch(
                            {
                                png(filename = paste0("enrichment_analysis", "/",fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".png"), width = ifelse(nrow(mat)<=20, 200, 125)+ncol(mat)*40, height = 50+nrow(mat)*50)
                            ...
                            error=function(e) {
                                print(paste0("Could not save enrichment_analysis", "/",fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".png because of the following error:\n", e))
                            }
                        )
                        tryCatch(
                            {
                                pdf(paste0("enrichment_analysis", "/", fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".pdf"), width = ifelse(nrow(mat)<=20, 50, 10)+ncol(mat)*0.1, height = 5+nrow(mat)*0.005)
                                pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values (z-scores) for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows)
                                dev.off()
                            },
                            error=function(e) {
                                print(paste0("Could not save enrichment_analysis", "/", fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".pdf because of the following error:\n", e))
                            }
                        )
                        tryCatch(
                            {
                                svg(paste0("enrichment_analysis", "/", fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".svg"), width = ifelse(nrow(mat)<=20, 50, 10)+ncol(mat)*0.1, height = 5+nrow(mat)*0.005)
                                pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values (z-scores) for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = params$heatmaps_cluster_cols, cluster_rows = params$heatmaps_cluster_rows)
                                dev.off()
                            },
                            error=function(e) {
                                print(paste0("Could not save enrichment_analysis", "/", fname, "/", "heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", make.names(pathway$term_id), ".svg because of the following error:\n", e))
                            }
                        )
                    ...
                    if (pathway$source == "KEGG"){
                        ...
                        tryCatch({
                            ...
                            if (length(entrez_ids)!=length(unique(entrez_ids))) {
                                ...
                                mv_command <- paste0("mv *.png *.xml ","./","enrichment_analysis", "/",fname, "/", "KEGG_pathways", "/")
                                rm_command <- paste0("rm ","./","enrichment_analysis", "/",fname, "/", "KEGG_pathways", "/", "*.xml")
                                system(mv_command)
                            }
                        }, error = function(e) {
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        # Create dir only if it is populated with plots
...
if (isProvided(params$set_background)) {
    background_list_text <- "The background gene list used for enrichment analysis is saved [here](./enrichment_analysis/metadata/custom_background.txt)"
} else {
...
if (mv_status) {
    if (isProvided(params$custom_gmt)) {
        gost_text <- "A custom GMT file was provided for the gost query; it is saved [here](./enrichment_analysis/metadata). In case this GMT file contained entries from datasources other than the selected ones, it was filtered and the resulting filtered GMT was used for querying. The filtered version is also saved to the metadata folder."
    } else {
        gost_text <- "A GMT file corresponding to the current gprofiler database version was downloaded to allow for reproducing the gost query; it is saved [here](./enrichment_analysis/metadata). Note that the downloaded GMT contains entries from various datasources, not just the selected ones. Also, some datasources might be missing from the files due to licensing reasons."
    }
} else {
...
## Enrichment analysis
The enrichment analysis was performed with the `gProfiler2` tool.

After unzipping the results ZIP file, the enrichment analysis results are stored in this folder: [enrichment_analysis](./enrichment_analysis), and explained in this section.
Inside the enrichment analysis results folder, a subfolder for each contrast used for the differential gene expression analysis is found. Inside each contrast folder, there is the following output:


<br>
Click to expand/uncollapse the details
<details>
<summary>**enrichment_analysis**</summary>
- **metadata**:
    - `custom_background.txt`: contains the list of genes to include for the enrichment analysis. If not provided as input, it will be computed by the pipeline and contains the genes that show expression values > 0 across all samples.
    - `gprofiler_*.gmt`: Gene Matrix Transposed file format (GMT) used by the pipeline. Can be downloaded manually from [gprofiler](https://biit.cs.ut.ee/gprofiler/gost) (Tab: Data sources) if the GMT file could not be downloaded automatically.
- **DE_contrast_condition_**:
    - `enrichment_plots/*_pathway_enrichment_plot.{pdf,png,svg}`: Barplots showing the proportion of differentially expressed genes in the pathway for a certain pathway database (might be missing if too many terms were enriched for fitting into a plot).
    - `gost_pathway_gostplot.{pdf/png/svg}`: Manhattan plots displaying all enriched pathways.
    - `*_gost_pathway_venn_diagram.{pdf,png}`: Venn diagrams showing the number of enriched terms when using a background gene list vs when not using a bg list.
    - `heatmaps/*.{pdf,svg,tsv}`: Contains heatmaps of the normalized DE gene counts for each enriched term.
    - `KEGG_pathways/`: Contains the KEGG pathways graphs with the log fold change of the differentially expressed genes.
    - `DATABASE_pathway_enrichment_results.tsv`: Table with the enriched terms for the specific database, containing the term name, code, number and names of DE genes in the pathway, fraction of DE genes in the pathway and adjusted p-value.
    - `pathway_enrichment_results.tsv`: Table with the enriched pathways for all databases, containing the term name and ID and further information.
</details><br>",
gost_text,

"\n
### Enrichment analysis {.tabset .tabset-pills}
The plot below summarizes the terms that were found significantly enriched in DE genes for each contrast (padj value <= ", pathway_pval_text, ").
Only contrasts for which enriched terms were found are shown.
Please hover over the dots to display the term names. The table below provides more detail on all enriched terms."))
```

```{r gost, eval=params$pathway_analysis, out.height = "100%", out.width = "100%", warning=FALSE, message=FALSE, results='asis'}
...
for (i in 1:length(q_names))  {
    ...
    if (length(q_list) > 0) {
        ...
        if (nrow(path_enrich) > 0){
            cat('#### Contrast: ', contrast_list[i], '\n')
            cat("\n")
            cat("Please hover over the datapoints to display additional information.")
            cat("\n\n")
            pg2 <- gostplot(gostres, capped=T, interactive=T) %>%
            layout(title = contrast_list[i],
            ...
            cat("\n\n")
            df_subset <- data.frame(Contrast = contrast_list[i],
                Pathway_name = path_enrich$term_name,
                Pathway_code = path_enrich$term_id,
                DE_genes = path_enrich$intersection_size,
                Pathway_size = path_enrich$term_size,
            ...
            cat('#### Contrast: ', contrast_list[i], '\n')
            cat("\n\n")
            cat(paste("> **ℹ️ Info:** No enriched terms were found in the selected databases."))
            path_enrich=data.frame(list())
            cat("\n\n")
        }
    } else {
        cat("> **⚠️ Warning:** No DE genes were found, enrichment analysis not possible.")
        path_enrich=data.frame(list())
    }
...
cat(paste0("### Enrichment terms heatmaps
For all considered contrasts, a heatmap of all DE genes that were found in each of the enriched terms is produced.
You can find these heatmaps inside the [enrichment_analysis](./enrichment_analysis/) folder, inside the subfolder corresponding to each of the contrasts,
and the subfolder: `heatmaps`.
\
"))
```
<!-- ET end -->
...
Primary bioinformatic analysis was performed using the `r primary_pipeline` pipeline and gene counts were obtained with the quantification method `r quant_tool`.

We analyzed the differential expression of genes in the provided data with the rnadeseq pipeline.

Genes were considered differentially expressed (DE) if their adjusted p-value was equal to or lower than `r pval_text` (padj <= `r pval_text`)
and if their log2FC was greater than `r logFC_text`. We compared the following conditions:

```{r summary, warning=FALSE, message=FALSE, results='asis'}
for (file in allgenes_files) {
    # Reading DE genes list
    fname <- tools::file_path_sans_ext(basename(file))
    DE_genes <- read.csv(file = paste0(wd, "/differential_gene_expression/allgenes/", file), sep="\t", header = T)
    ...
    contrast_len <- sapply(strsplit(fname, "_"), length)
    current_condition <- sapply(strsplit(fname, '_'), function(x) paste(x[4], collapse=' '))
    current_contrast_name <- sapply(strsplit(fname, '_'), function(x) paste(x[(contrast_len-2):contrast_len], collapse=' '))
    cat(paste("* Comparison:", current_condition, ":", current_contrast_name, "has", numDE, "DE genes out of", num_all, "tested genes.\n"))
}
```
...
```{r software_versions_duplicate_chunk, bootstrap.show.message=FALSE, echo=FALSE, message=FALSE, eval=isProvided(params$path_software_versions)}
# Import the .csv or .yml file output by the rna-seq Nextflow pipeline
software_versions_csv <- endsWith(params$path_software_versions, ".csv") || endsWith(params$path_software_versions, ".tsv")
if (software_versions_csv) {
    version_csv <- read.csv(file = params$path_software_versions, sep="\t", header = FALSE)
...
if (software_versions_csv) {
    if (params$input_type == "featurecounts") {
        quant_tool <- "[featureCounts]"
        quant_link <- "(https://subread.sourceforge.net/featureCounts.html)"
        quant_version <- as.character(version["featureCounts"])
        quant_cite <- "[@liao2014featurecounts]"
    }
} else {
    if (params$input_type == "rsem") {
        quant_tool <- "[RSEM]"
        quant_link <- "(https://github.com/deweylab/RSEM)"
        quant_version <- as.character(version["RSEM_CALCULATEEXPRESSION"][[1]]["rsem"])
        quant_cite <- "[@li2011rsem]"
    } else if (params$input_type == "salmon") {
        quant_tool <- "[Salmon]"
        quant_link <- "(https://salmon.readthedocs.io/en/latest/salmon.html)"
        quant_version <- as.character(version["SALMON_QUANT"][[1]]["salmon"])
        quant_cite <- "[@patro2017salmon]"
    } else if (params$input_type == "smrnaseq") {
        quant_tool <- "[SAMtools]"
        quant_link <- "(https://www.htslib.org/)"
        quant_version <- as.character(version["SAMTOOLS_IDXSTATS"][[1]]["samtools"])
        quant_cite <- "[@li2009sequence; @li2011statistical]"
    }
}
...
# In the following part, I use newlines (in code) where possible whenever the paste0 function switches from a string to a variable call to
# better distinguish between the two so it is clearer in which lines quotes are needed and in which not
# --> Cannot always use newlines as this sometimes translates into the report.html

if (params$input_type == "featurecounts" || params$input_type == "salmon" || params$input_type == "rsem"){
cat(paste0(
"## Primary Bioinformatics

### RNASeq analysis {#rnaseq-methods}

The Nextflow-based [nf-core/rnaseq](https://nf-co.re/rnaseq) pipeline, version ", as.character(ifelse(software_versions_csv, version["nf-core/rnaseq"],
version["Workflow"][[1]]["nf-core/rnaseq"])),
", was used for the [primary bioinformatic analysis](#primary-bioinformatics-rnaseq).
[FASTQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) ", as.character(ifelse(software_versions_csv, version["FastQC"], version["FASTQC"][[1]]["fastqc"])),
" [@andrews2010fastqc] was used to determine the quality of the FASTQ files.
Subsequently, adapter trimming was conducted with [Trim Galore](https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) ",
as.character(ifelse(software_versions_csv, version["Trim Galore!"], version["TRIMGALORE"][[1]]["trimgalore"])),
" [@krueger2012trim].
The [STAR](https://github.com/alexdobin/STAR) v", as.character(ifelse(software_versions_csv, substring((version["STAR"]), 7, ), version["MAKE_TRANSCRIPTS_FASTA"][[1]]["star"])),
" [@Dobin2013] aligner was used to map the reads that passed the quality control to the reference genome.
The RNA-seq data quality control was performed with [RSeQC](https://rseqc.sourceforge.net/) ",
...
### smRNA-seq data analysis {#smrnaseq-methods}

The Nextflow-based [nf-core/smrnaseq](https://nf-co.re/smrnaseq/) pipeline, version ", as.character(version["Workflow"][[1]]["nf-core/smrnaseq"]),
" was used for the [primary bioinformatic analysis](#primary-bioinformatics-smrnaseq).
[FASTQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) ", as.character(version["FASTQC"][[1]]["fastqc"]),
" [@andrews2010fastqc] was used to determine the quality of the miRNA samples. ",
ifelse("TRIMGALORE" %in% names(version), paste0("Subsequently, adapter trimming was conducted with [Trim Galore](https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/) ",
as.character(version["TRIMGALORE"][[1]]["trimgalore"]),
" [@krueger2012trim]."),
    ifelse("FASTP" %in% names(version), paste0("Subsequently, adapter trimming was conducted with [fastP](https://github.com/OpenGene/fastp) ",
    as.character(version["FASTP"][[1]]["fastp"]),
    " [@chen2018fastp; @chen2023ultrafast]."),
        "")),
" [Bowtie](https://bowtie-bio.sourceforge.net/bowtie2/index.html)", as.character(version["BOWTIE_MAP_HAIRPIN"][[1]]["bowtie"]), " [@langmead2009ultrafast]
aligner was used to map the reads that passed the quality control to the reference database.
The smRNA-seq data quality control was performed with [miRTrace](https://github.com/friedlanderlab/mirtrace) ",
as.character(version["MIRTRACE_RUN"][[1]]["mirtrace"]),
" [@kang2018mirtrace] and read quantification of the features (e.g. genes) with ",
quant_tool,
quant_link,
" ",
quant_version,
...
quant_cite,
".",
"\nThe output from [SAMtools](https://www.htslib.org/): `<QBiC-Code>_mature_hairpin.sorted.idxstats` and `<QBiC-Code>_mature.sorted.idxstats` files,
were then used to create a count matrix. This was done by combining all of these files into a single table, using only the rownames and the third
column (second column after the rownames) of each file which contains the counts.
The last row of each file (which always has an asterisk `*` as rowname) was excluded.
The resulting count matrix was used as input for the [rnadeseq pipeline](https://github.com/qbic-pipelines/rnadeseq)."
))
}
```
...
cat(paste0("\nAn aggregation of the quality control for the RNAseq analysis was performed with [MultiQC](https://seqera.io/multiqc/) ", as.character(version["MultiQC"]),
"[@ewels2016multiqc]."))
```
...
The secondary analysis of differential gene expression was performed by using the [rnadeseq pipeline](https://github.com/qbic-pipelines/rnadeseq), version ",
as.character(params$revision),
", which was written using the nf-core template [@ewels2020nf]. The read quantification data resulting from ",
quant_tool, quant_link, " was processed with the R package [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html) v",
packageVersion("DESeq2"),
" [@love2014differential].
Genes are considered differentially expressed if they show an adjusted p-value<=",
pval_text, " and a log2 Fold Change>=",
logFC_text, "."
))
...
if (isProvided(params$custom_gmt)){
    ...
    database_string <- paste0(database_string, "gprofiler2: ", database_version_info$gprofiler_version)
    for (db in datasources) {
        database_string <- paste0(database_string, ", ", db, ": ", database_version_info$sources[[db]]$version)
    }
} else {
    database_string <- paste("All datasources available in g:Profiler were considered (check https://biit.cs.ut.ee/gprofiler/gost and
    https://biit.cs.ut.ee/gprofiler/page/docs for more info).")
}

cat(paste0(
"\n### Enrichment analysis

For the enrichment analysis, the R packages [gprofiler2](https://biit.cs.ut.ee/gprofiler/page/r) v", version_gprofiler2,
", [AnnotationDbi](https://bioconductor.org/packages/release/bioc/html/AnnotationDbi.html) v", version_annotation,
" and  the organism ", name_species, " v", version_annotation,
" were used. ", database_string, ".\n",
"Enriched terms were classified as enriched for genes with an adjusted p-value <= ", pathway_pval_text, "."
))
```

All images in this report were generated in RStudio with `r R.version.string` mainly using the R package [ggplot2](https://ggplot2.tidyverse.org/) v`r packageVersion("ggplot2")` [@ggplot].
Final reports were produced using the R package [rmarkdown](https://rmarkdown.rstudio.com/) v`r packageVersion("rmarkdown")` , with the [knitr](https://cran.r-project.org/web/packages/knitr/index.html) v`r packageVersion("knitr")`
and [DT](https://rstudio.github.io/DT/) v`r packageVersion("DT")` R packages.

## Package version overview

The versions of the software used for the analysis are saved in the [pipeline_info](../../pipeline_info/) folder.
The following section lists all packages used for analysis, together with their versions.

```{r rnadeseq_versions_output, echo=FALSE, message=FALSE, warning=F}
sessioninfo::session_info() %>%  write_yaml("rnadeseq_software_versions.yml")
```
